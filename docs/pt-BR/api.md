...
class Request:
    ...
    def __del__(self):
```
## Opções de aplicações para ouvir
```python
...
class WebSocket:
    ...
    # UUID para dados do socket, usado para liberar dados após o socket ser fechado.
    def get_user_data_uuid(self):
    def get_user_data(self):
    # Clonar a instância atual para preservá-la (você precisa observar conexão fechadas ao usá-la)
    def clone(self):
...
## Enumerações
```python
# O modo compressor tem 8 bits mais baixos, onde HIGH4(windowBits), LOW4 (MemLevel)
# O modo descompressor tem 8 bits mais altos LOW4 (windowsBits)
# Se os bits do compressor ou descompressor forem 1, eles serão compartilhados.
# Se tudo for simplesmente 0, então tudo está desabilitado.
class CompressOptions(IntEnum):
    # Desabilidado, compartilhado, compatilhado são valores "especiais"
    DISABLED = lib.DISABLED
    SHARED_COMPRESSOR = lib.SHARED_COMPRESSOR
    SHARED_DECOMPRESSOR = lib.SHARED_DECOMPRESSOR
    # Os 4 bits mais altos descrevem o descompressor
    ...
    # Compressor de descrição mais baixo de 8 bits.
